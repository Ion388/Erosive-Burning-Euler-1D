...
static void enforce_positive_state(double* U, const double* A, const double* k, const SolverWorkspace* ws) {
    ...
    for (int i = 0; i < nx; ++i) {
        double Ause = MAXD(A[i], area_floor);
        double rho = MAXD(U[IDX(0, i, nx)] / Ause, rho_floor);
        double rho_Ause = rho * Ause;
        double u = U[IDX(1, i, nx)] / rho_Ause;

        double kminus1 = MAXD(k[i] - 1.0, 1e-8);
        double E = U[IDX(2, i, nx)] / rho_Ause;
        double p = rho * kminus1 * (E - 0.5 * u * u);
        p = MAXD(p, p_floor);
        double E_final = p / (rho * kminus1) + 0.5 * u * u;
...
static void primitives_general(const double* U, int nU, const double* A, int nA, const double* k,
    ...
    for (int i = 0; i < nU; ++i) {
        ...
        double rhoi = MAXD(U[IDX(0, i, nU)] / Ause, rho_floor);
        double rho_A = rhoi * Ause;
        double ui = U[IDX(1, i, nU)] / rho_A;
        double Ei = U[IDX(2, i, nU)] / rho_A;

        double kminus1 = MAXD(k[i] - 1.0, 1e-8);
        double pi = rhoi * kminus1 * (Ei - 0.5 * ui * ui);
        pi = MAXD(pi, p_floor);
        double Efi = pi / (rhoi * kminus1) + 0.5 * ui * ui;
        double a2 = MAXD(k[i] * pi / rhoi, a2_floor);
...
static void weno5_reconstruct(const double* U, const double* A, const double* k,
    ...
    double A_im1, A_i, rhoL, velL, EL, kL, pL, HL, rhoR, velR, ER, kR, pR, HR, sL, sR, denom, inv_denom, rhohat, uhat, Hhat, kh, ahat2, ahat;

    double Pm[9];
    double Lm[9];

    double u_im3[3], u_im2[3], u_im1[3], u_i[3], u_ip1[3], u_ip2[3];
    double wm_im3[3], wm_im2[3], wm_im1[3], wm_i[3], wm_ip1[3], wm_ip2[3];
    double w_uL[3], w_uR[3];

    double p0L, p1L, p2L, p0R, p1R, p2R, dwL1, dwL2, dwL3, dwR1, dwR2, dwR3, alpha1L, alpha2L, alpha3L, alpha1R, alpha2R, alpha3R;
    double w1L, w2L, w3L, w1R, w2R, w3R, beta1L, beta2L, beta3L, beta1R, beta2R, beta3R, sumL, sumR;
    double ULv[3], URv[3];
        
    #pragma omp parallel for private(c, j, im3, im2, im1, i0, ip1, ip2, A_im1, A_i, rhoL, velL, EL, kL, pL, HL, rhoR, velR, ER, kR, pR, HR, sL, sR, denom, inv_denom, \
        rhohat, uhat, Hhat, kh, ahat2, ahat, Pm, Lm, u_im3, u_im2, u_im1, u_i, u_ip1, u_ip2, wm_im3, wm_im2, wm_im1, wm_i, wm_ip1, wm_ip2, \
        w_uL, w_uR, p0L, p1L, p2L, p0R, p1R, p2R, dwL1, dwL2, dwL3, dwR1, dwR2, dwR3, alpha1L, alpha2L, alpha3L, alpha1R, alpha2R, alpha3R, \
        w1L, w2L, w3L, w1R, w2R, w3R, beta1L, beta2L, beta3L, beta1R, beta2R, beta3R, sumL, sumR, ULv, URv) shared(U, A, k, UL, UR, khat, ws) schedule(static)
    for (int j = 0; j < m; ++j) {
        int im3 = j;
        int im2 = j + 1;
        int im1 = j + 2;
        ...
        A_im1 = MAXD(A[im1], ws->area_floor);
        A_i = MAXD(A[i0], ws->area_floor);

        rhoL = MAXD(U[IDX(0, im1, nx)] / A_im1, ws->rho_floor);
        velL = U[IDX(1, im1, nx)] / (rhoL * A_im1);
        EL = U[IDX(2, im1, nx)] / (rhoL * A_im1);
        kL = k[im1];
        pL = rhoL * MAXD(kL - 1.0, 1e-8) * (EL - 0.5 * velL * velL);
        pL = MAXD(pL, ws->p_floor);
        HL = EL + pL / rhoL;

        rhoR = MAXD(U[IDX(0, i0, nx)] / A_i, ws->rho_floor);
        velR = U[IDX(1, i0, nx)] / (rhoR * A_i);
        ER = U[IDX(2, i0, nx)] / (rhoR * A_i);
        kR = k[i0];
        pR = rhoR * MAXD(kR - 1.0, 1e-8) * (ER - 0.5 * velR * velR);
        pR = MAXD(pR, ws->p_floor);
        HR = ER + pR / rhoR;

        sL = sqrt(MAXD(rhoL, ws->rho_floor));
        sR = sqrt(MAXD(rhoR, ws->rho_floor));
        denom = regularize_denom(sL + sR, 1e-12);
        inv_denom = 1.0 / denom;

        rhohat = sL * sR;
        uhat = (sL * velL + sR * velR) * inv_denom;
        Hhat = (sL * HL + sR * HR) * inv_denom;
        kh = (sL * kL + sR * kR) * inv_denom;

        ahat2 = (kh - 1.0) * (Hhat - 0.5 * uhat * uhat);
        ...
        u_im3[0] = U[IDX(0, im3, nx)];
        u_im3[1] = U[IDX(1, im3, nx)];
        u_im3[2] = U[IDX(2, im3, nx)];
        u_im2[0] = U[IDX(0, im2, nx)];
        u_im2[1] = U[IDX(1, im2, nx)];
        u_im2[2] = U[IDX(2, im2, nx)];
        u_im1[0] = U[IDX(0, im1, nx)];
        u_im1[1] = U[IDX(1, im1, nx)];
        u_im1[2] = U[IDX(2, im1, nx)];
        u_i[0] = U[IDX(0, i0, nx)];
        u_i[1] = U[IDX(1, i0, nx)];
        u_i[2] = U[IDX(2, i0, nx)];
        u_ip1[0] = U[IDX(0, ip1, nx)];
        u_ip1[1] = U[IDX(1, ip1, nx)];
        u_ip1[2] = U[IDX(2, ip1, nx)];
        u_ip2[0] = U[IDX(0, ip2, nx)];
        u_ip2[1] = U[IDX(1, ip2, nx)];
        u_ip2[2] = U[IDX(2, ip2, nx)];
        ...
        for (c = 0; c < 3; ++c) {
            p0L = (1.0 / 3.0) * wm_im3[c] - (7.0 / 6.0) * wm_im2[c] + (11.0 / 6.0) * wm_im1[c];
            p1L = -(1.0 / 6.0) * wm_im2[c] + (5.0 / 6.0) * wm_im1[c] + (1.0 / 3.0) * wm_i[c];
            p2L = (1.0 / 3.0) * wm_im1[c] + (5.0 / 6.0) * wm_i[c] - (1.0 / 6.0) * wm_ip1[c];

            dwL1 = wm_im3[c] - 2.0 * wm_im2[c] + wm_im1[c];
            dwL2 = wm_im2[c] - 2.0 * wm_im1[c] + wm_i[c];
            dwL3 = wm_im1[c] - 2.0 * wm_i[c] + wm_ip1[c];

            beta1L = (13.0 / 12.0) * dwL1 * dwL1 + 0.25 * (wm_im3[c] - 4.0 * wm_im2[c] + 3.0 * wm_im1[c]) * (wm_im3[c] - 4.0 * wm_im2[c] + 3.0 * wm_im1[c]);
            beta2L = (13.0 / 12.0) * dwL2 * dwL2 + 0.25 * (wm_im2[c] - wm_i[c]) * (wm_im2[c] - wm_i[c]);
            beta3L = (13.0 / 12.0) * dwL3 * dwL3 + 0.25 * (3.0 * wm_im1[c] - 4.0 * wm_i[c] + wm_ip1[c]) * (3.0 * wm_im1[c] - 4.0 * wm_i[c] + wm_ip1[c]);

            alpha1L = 0.1 / ((eps_weno + beta1L) * (eps_weno + beta1L));
            alpha2L = 0.6 / ((eps_weno + beta2L) * (eps_weno + beta2L));
            alpha3L = 0.3 / ((eps_weno + beta3L) * (eps_weno + beta3L));
            sumL = alpha1L + alpha2L + alpha3L;
            w1L = alpha1L / sumL;
            w2L = alpha2L / sumL;
            w3L = 1.0 - w1L - w2L;
            w_uL[c] = w1L * p0L + w2L * p1L + w3L * p2L;

            p0R = -(1.0 / 6.0) * wm_im2[c] + (5.0 / 6.0) * wm_im1[c] + (1.0 / 3.0) * wm_i[c];
            p1R = (1.0 / 3.0) * wm_im1[c] + (5.0 / 6.0) * wm_i[c] - (1.0 / 6.0) * wm_ip1[c];
            p2R = (11.0 / 6.0) * wm_i[c] - (7.0 / 6.0) * wm_ip1[c] + (1.0 / 3.0) * wm_ip2[c];

            dwR1 = wm_im2[c] - 2.0 * wm_im1[c] + wm_i[c];
            dwR2 = wm_im1[c] - 2.0 * wm_i[c] + wm_ip1[c];
            dwR3 = wm_i[c] - 2.0 * wm_ip1[c] + wm_ip2[c];

            beta1R = (13.0 / 12.0) * dwR1 * dwR1 + 0.25 * (wm_im2[c] - 4.0 * wm_im1[c] + 3.0 * wm_i[c]) * (wm_im2[c] - 4.0 * wm_im1[c] + 3.0 * wm_i[c]);
            beta2R = (13.0 / 12.0) * dwR2 * dwR2 + 0.25 * (wm_im1[c] - wm_ip1[c]) * (wm_im1[c] - wm_ip1[c]);
            beta3R = (13.0 / 12.0) * dwR3 * dwR3 + 0.25 * (3.0 * wm_i[c] - 4.0 * wm_ip1[c] + wm_ip2[c]) * (3.0 * wm_i[c] - 4.0 * wm_ip1[c] + wm_ip2[c]);

            alpha1R = 0.3 / ((eps_weno + beta1R) * (eps_weno + beta1R));
            alpha2R = 0.6 / ((eps_weno + beta2R) * (eps_weno + beta2R));
            alpha3R = 0.1 / ((eps_weno + beta3R) * (eps_weno + beta3R));
            sumR = alpha1R + alpha2R + alpha3R;
            w1R = alpha1R / sumR;
            w2R = alpha2R / sumR;
            w3R = 1.0 - w1R - w2R;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (ts->hist_len > 0) {
            for (int j = 0; j < ts->hist_len; ++j) {
                ...
                history_term += ts->q_hist[(size_t)j * (size_t)n + (size_t)i] * kernel;
            }
        }
        ...
        ts->q_hist[(size_t)ts->hist_len * (size_t)n + (size_t)i] = q_new;
    }
