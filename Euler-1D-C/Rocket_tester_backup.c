...
static void weno5_reconstruct(const double* U, const double* A, const double* k,
    ...
    for (int j = 0; j < m; ++j) {
        ...
        double u_im3[3] = {U[IDX(0, im3, nx)], U[IDX(1, im3, nx)], U[IDX(2, im3, nx)]};
        double u_im2[3] = {U[IDX(0, im2, nx)], U[IDX(1, im2, nx)], U[IDX(2, im2, nx)]};
        double u_im1[3] = {U[IDX(0, im1, nx)], U[IDX(1, im1, nx)], U[IDX(2, im1, nx)]};
        double u_i[3] = {U[IDX(0, i0, nx)], U[IDX(1, i0, nx)], U[IDX(2, i0, nx)]};
        double u_ip1[3] = {U[IDX(0, ip1, nx)], U[IDX(1, ip1, nx)], U[IDX(2, ip1, nx)]};
        double u_ip2[3] = {U[IDX(0, ip2, nx)], U[IDX(1, ip2, nx)], U[IDX(2, ip2, nx)]};
        ...
        for (int c = 0; c < 3; ++c) {
            ...
            double beta1L = (13.0 / 12.0) * dwL1 * dwL1 + 0.25 * (wm_im3[c] - 4.0 * wm_im2[c] + 3.0 * wm_im1[c]) * (wm_im3[c] - 4.0 * wm_im2[c] + 3.0 * wm_im1[c]);
            double beta2L = (13.0 / 12.0) * dwL2 * dwL2 + 0.25 * (wm_im2[c] - wm_i[c]) * (wm_im2[c] - wm_i[c]);
            double beta3L = (13.0 / 12.0) * dwL3 * dwL3 + 0.25 * (3.0 * wm_im1[c] - 4.0 * wm_i[c] + wm_ip1[c]) * (3.0 * wm_im1[c] - 4.0 * wm_i[c] + wm_ip1[c]);
            ...
            double beta1R = (13.0 / 12.0) * dwR1 * dwR1 + 0.25 * (wm_im2[c] - 4.0 * wm_im1[c] + 3.0 * wm_i[c]) * (wm_im2[c] - 4.0 * wm_im1[c] + 3.0 * wm_i[c]);
            double beta2R = (13.0 / 12.0) * dwR2 * dwR2 + 0.25 * (wm_im1[c] - wm_ip1[c]) * (wm_im1[c] - wm_ip1[c]);
            double beta3R = (13.0 / 12.0) * dwR3 * dwR3 + 0.25 * (3.0 * wm_i[c] - 4.0 * wm_ip1[c] + wm_ip2[c]) * (3.0 * wm_i[c] - 4.0 * wm_ip1[c] + wm_ip2[c]);
