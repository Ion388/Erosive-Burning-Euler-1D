...
contains
	...
	subroutine enforce_positive_state(U, A_loc, k)
		...
		real(dp) :: Ause, rho, rhoA, vel, Eint, pval, kminus1
		integer :: j
		! $omp parallel do default(shared) private(j, Ause, rho, rhoA, vel, Eint, pval, kminus1) schedule(static)
		do j = 1, nx
			Ause = max(A_loc(j), area_floor)
			rho = max(U(1, j) / Ause, rho_floor)
			rhoA = rho * Ause
			vel = U(2, j) / max(rhoA, 1.0e-20_dp)
			Eint = U(3, j) / max(rhoA, 1.0e-20_dp)
			kminus1 = max(k(j) - 1.0_dp, 1.0e-8_dp)
			pval = rho * kminus1 * (Eint - 0.5_dp * vel * vel)
			pval = max(pval, p_floor)
			Eint = pval / (rho * kminus1) + 0.5_dp * vel * vel
	...
	subroutine primitives(Uin, Ain, kin, rho, vel, pval, Eout, a)
		real(dp), intent(in) :: Uin(:, :), Ain(:), kin(:)
		real(dp), intent(out) :: rho(size(Uin, 2)), vel(size(Uin, 2)), pval(size(Uin, 2)), Eout(size(Uin, 2)), a(size(Uin, 2))
		real(dp) :: Ause(size(Uin, 2)), kminus1, u2, a2
		integer :: nloc, j

		nloc = size(Uin, 2)
		if (size(Ain) == nloc) then
			Ause = Ain
		else
			do j = 1, nloc
				Ause(j) = 0.5_dp * (Ain(j + 2) + Ain(j + 3))
			end do
		end if
		! $omp parallel do default(shared) private(j, kminus1, u2, a2) schedule(static)
		do j = 1, nloc
			Ause(j) = max(Ause(j), area_floor)
			rho(j) = max(Uin(1, j) / Ause(j), rho_floor)
			vel(j) = Uin(2, j) / max(rho(j) * Ause(j), 1.0e-20_dp)
			Eout(j) = Uin(3, j) / max(rho(j) * Ause(j), 1.0e-20_dp)
			kminus1 = max(kin(j) - 1.0_dp, 1.0e-8_dp)
			u2 = vel(j) * vel(j)
			pval(j) = rho(j) * kminus1 * (Eout(j) - 0.5_dp * u2)
			pval(j) = max(pval(j), p_floor)
			Eout(j) = pval(j) / (rho(j) * kminus1) + 0.5_dp * u2
			a2 = max(kin(j) * pval(j) / rho(j), a2_floor)
			a(j) = sqrt(a2)
		end do
		! $omp end parallel do
	...
	subroutine weno5_reconstruct(U, A_loc, k, UL, UR, eigenvals, khat_out)
		...
		integer :: j, r, tile, jbeg, jend

		eps_weno = 1.0e-6_dp

		n_ifaces = nx - 5
    	n_tiles = max(1, n_procs)
	...
	subroutine max_wave_speed_toro(U, A_loc, k, SL, SR, maxabs)
		...
		maxabs = 0.0_dp
		! $omp parallel do default(shared) private(tile, jbeg, jend, j, khat_safe, kh_m1, kh_p1, inv2k, gexp, pexp, pL_gamma, pR_gamma, &
		! $omp&     base_num, base_den, base, pstarr, coeff, qL, qR) schedule(static)
		do tile = 1, n_tiles
			jbeg = 1 + (tile - 1) * iface_group_size
			jend = min(n_ifaces, jbeg + iface_group_size - 1)
			do j = jbeg, jend
	...
	subroutine euler_flux(Uin, A_loc, k, flux)
		...
		nloc = size(Uin, 2)
		call primitives(Uin, A_loc, k, rho, vel, pval, Eout, a)
	...
	subroutine detect_troubled_interfaces(U, A_loc, k, troubled)
		...
		integer :: n_ifaces, n_tiles, iface_group_size
		real(dp) :: UL(3, nx - 5), UR(3, nx - 5), eig(3, nx - 5), khat(nx - 5)
		real(dp) :: Ause_l, Ause_r, rhoL_u, rhoR_u, rhoL, rhoR
		...
		call weno5_reconstruct(U, A_loc, k, UL, UR, eig, khat)
		troubled = .false.
		! $omp parallel do default(shared) private(tile, jbeg, jend, j, Ause_l, Ause_r, rhoL_u, rhoR_u, rhoL, rhoR, velL2, velR2, EL, ER, kLm1, kRm1, pL_u, pR_u, psum, jump) schedule(static)
		do tile = 1, n_tiles
			jbeg = 1 + (tile - 1) * iface_group_size
			jend = min(n_ifaces, jbeg + iface_group_size - 1)
			do j = jbeg, jend
			...
			rhoL = max(rhoL_u, rho_floor)
			rhoR = max(rhoR_u, rho_floor)
			velL2 = (UL(2, j) / max(rhoL * Ause_l, 1.0e-20_dp)) ** 2
			velR2 = (UR(2, j) / max(rhoR * Ause_r, 1.0e-20_dp)) ** 2
			EL = UL(3, j) / max(rhoL * Ause_l, 1.0e-20_dp)
			ER = UR(3, j) / max(rhoR * Ause_r, 1.0e-20_dp)
			kLm1 = max(k(j + 2) - 1.0_dp, 1.0e-8_dp)
			kRm1 = max(k(j + 3) - 1.0_dp, 1.0e-8_dp)
			pL_u = rhoL * kLm1 * (EL - 0.5_dp * velL2)
			pR_u = rhoR * kRm1 * (ER - 0.5_dp * velR2)
	...
	subroutine hlle_flux(U, A_loc, k, fm, fp)
		...
		real(dp) :: SL(nx - 5), SR(nx - 5), maxabs, denom
		integer :: j, tile, jbeg, jend
		...
		! $omp parallel do default(shared) private(tile, jbeg, jend, j, denom) schedule(static)
		do tile = 1, n_tiles
			jbeg = 1 + (tile - 1) * iface_group_size
			jend = min(n_ifaces, jbeg + iface_group_size - 1)
			do j = jbeg, jend
			denom = regularize_denom(SR(j) - SL(j))
			fhlle(:, j) = (SR(j) * fL(:, j) - SL(j) * fR(:, j) + SL(j) * SR(j) * (UR(:, j) - UL(:, j))) / denom
			end do
		end do
		! $omp end parallel do
	...
	subroutine hllc_flux(U, A_loc, k, fm, fp)
		...
		real(dp) :: Ause, denom_sstar, denom_l, denom_r, denom_e_l, denom_e_r, denom
		...
		! $omp parallel do default(shared) private(tile, jbeg, jend, j, Ause, denom_sstar, denom_l, denom_r, denom_e_l, denom_e_r) schedule(static)
		do tile = 1, n_tiles
			jbeg = 1 + (tile - 1) * iface_group_size
			jend = min(n_ifaces, jbeg + iface_group_size - 1)
			do j = jbeg, jend
		...
		do j = 1, nx - 5
			if (troubled(j)) then
				...
				if (j < nx - 5) theta(j + 1) = min(theta(j + 1), 0.5_dp)
			end if
		end do
	...
	subroutine ssprk45(U, A_loc, P_loc, state, dt_local, dx_local, t_local, k, erosive_on, Unp1, Anew, Pnew)
		...
		k1 = -(fp - fm) / dx_local + S
		U1(:, 4:nx - 3) = U0(:, 4:nx - 3) + 0.391752226571890_dp * dt_local * k1
		...
		k2 = -(fp - fm) / dx_local + S
		U2(:, 4:nx - 3) = 0.444370493651235_dp * U0(:, 4:nx - 3) + &
						  0.555629506348765_dp * U1(:, 4:nx - 3) + &
						  0.368410593050371_dp * dt_local * k2
		...
		k3 = -(fp - fm) / dx_local + S
		U3(:, 4:nx - 3) = 0.620101851488403_dp * U0(:, 4:nx - 3) + &
						  0.379898148511597_dp * U2(:, 4:nx - 3) + &
						  0.251891774271694_dp * dt_local * k3
		...
		k4 = -(fp - fm) / dx_local + S
		U4(:, 4:nx - 3) = 0.178079954393132_dp * U0(:, 4:nx - 3) + &
						  0.821920045606868_dp * U3(:, 4:nx - 3) + &
						  0.544974750228521_dp * dt_local * k4
		...
		Unp1(:, 4:nx - 3) = 0.517231671970585_dp * U2(:, 4:nx - 3) + 0.096059710526147_dp * U3(:, 4:nx - 3) + &
												0.063692468666290_dp * dt_local * k4 + 0.386708617503268_dp * U4(:, 4:nx - 3) + 0.226007483236906_dp * dt_local * k5
