...
def simulate():
    ...
    def apply_nozzle_outlet(U, A, k):
        ...
        # Mach number at nozzle inlet must be greater than M_compare for the flow to be supersonic at the throat
        M_compare = solve_area_mach(area_ratio, k_f, supersonic=False)
        # print(M_compare)
        if M_s < M_compare:
            return apply_atmospheric_outlet(U, A, k)
        ...
        E_s = p_s / (k_s - 1.0) + 0.5 * rho_s * u_s * u_s
    ...
    def update_ignition_thermal_state(U, A, P, thermal_state, dt, k):
        ...
        # Gnielinski
        # Re_safe = np.maximum(Re, 1.0 + 1e-12)
        # Pr_safe = np.maximum(Pr, 1e-12)
        # # Darcy friction factor for smooth tubes (valid in turbulent regime)
        # f = (0.79 * np.log(Re_safe) - 1.64) ** (-2.0)
        # # Gnielinski correlation
        # Nu_gn = ((f / 8.0) * (Re_safe - 1000.0) * Pr_safe) / (
        #     1.0 + 12.7 * np.sqrt(f / 8.0) * (Pr_safe ** (2.0 / 3.0) - 1.0)
        # )
        # # Keep your laminar fallback
        # Nu = np.where(Re_safe >= 3000.0, Nu_gn, 3.66)
        # # Heat transfer coefficient
        # htc = Nu * K / np.maximum(Dh, 1e-20)

        # print(np.max(Pr), np.min(Pr))
        # print(np.max(Re), np.min(Re))
        # print where Re is max and min
        # print(np.unravel_index(np.argmax(Re), Re.shape), np.unravel_index(np.argmin(Re), Re.shape))
