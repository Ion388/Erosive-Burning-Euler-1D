program rocket_tester
	...
	! write(geom_filename, '("riemann_geometry_history_case", I0, ".dat")') case_id
	...
	write(thermo_filename, '("riemann_state_history_case", I0, ".dat")') case_id
	...
	! write(exit_filename, '("riemann_exit_history_case", I0, ".dat")') case_id
	...
	do while (t < t_end)
		...
		if (mod(n, geom_write_stride) == 0) then
			! write(geom_unit, *) t, A, P
		end if
	...
	! write(prof_filename, '("riemann_profiles_case", I0, ".dat")') case_id
...
contains
	...
	subroutine initial_riemann(U, A_loc, left_state, right_state, k)
		...
		real(dp) :: rhoL, pL, uL, rhoR, pR, uR, kL, kR, eL, eR, pC, eC
		integer :: j, mid
		...
		if (mod(nx, 2) == 0) then
			...
			do j = 1, nx
				if (j <= mid) then
					...
					U(1, j) = (rhoR * A_loc(j) + rhoL * A_loc(j))/2.0_dp
					U(2, j) = (rhoR * uR * A_loc(j) + rhoL * uL * A_loc(j))/2.0_dp
					pC = (pR + pL)/2.0_dp
					eC = pC / (0.5_dp * (kL + kR) - 1.0_dp) + ((uL + uR) / 2.0_dp) ** 2 * (rhoR + rhoL)/2.0_dp
					U(3, j) = eC * A_loc(j)
					!  U(3, j) = (eR * A_loc(j) + eL * A_loc(j))/2.0_dp
				else
					U(1, j) = rhoR * A_loc(j)
					U(2, j) = rhoR * uR * A_loc(j)
					U(3, j) = eR * A_loc(j)
	...
	subroutine primitives(Uin, Ain, kin, rho_out, vel, pval, Eout, a)
		...
		do j = 1, nloc
			...
			vel(j) = Uin(2, j) / rho_out(j) * Ause_j
			Eout(j) = Uin(3, j) / rho_out(j) * Ause_j
	...
	subroutine weno5_reconstruct(U, A_loc, k, UL, UR, eigenvals, khat_out)
		...
		do j = 1, n_ifaces
			...
			rhoL = uim1(1) / A_im1
			velL = uim1(2) / rhoL * A_im1
			EL = uim1(3) / rhoL * A_im1
			kL = k(j + 2)
			kLm1 = kL - 1.0_dp
			pL = rhoL * kLm1 * (EL - 0.5_dp * velL * velL)
			HL = EL + pL / rhoL
			...
			kR = k(j + 3)
			kRm1 = kR - 1.0_dp
			pR = rhoR * kRm1 * (ER - 0.5_dp * velR * velR)
			HR = ER + pR / rhoR
			...
			ahat2 = (khat - 1.0_dp) * (Hhat - 0.5_dp * uhat * uhat)
			ahat = sqrt(ahat2)
	...
	subroutine max_wave_speed_toro(U, A_loc, k, SL, SR, maxabs)
		...
		do j = 1, n_ifaces
			...
			base = base_num / regularize_denom(base_den)
			pstarr = base ** pexp
			...
			if (pstarr <= pL_arr(j)) then
				...
				qL = sqrt(1.0_dp + coeff * (pstarr / pL_arr(j) - 1.0_dp))
			end if
			if (pstarr <= pR_arr(j)) then
				qR = 1.0_dp
			else
				qR = sqrt(1.0_dp + coeff * (pstarr / pR_arr(j) - 1.0_dp))
			end if
	...
	subroutine wave_speeds_from_prims(rhoL_arr, velL_arr, pL_arr, aL_arr, &
		...
		do j = 1, n_ifaces
			...
			if (pstarr <= pL_j) then
				...
				qL = sqrt(1.0_dp + coeff * (pstarr / pL_j - 1.0_dp))
			end if
			if (pstarr <= pR_j) then
				qR = 1.0_dp
			else
				qR = sqrt(1.0_dp + coeff * (pstarr / pR_j - 1.0_dp))
			end if
			SL(j) = velL_arr(j) - aL_arr(j) * qL
			SR(j) = velR_arr(j) + aR_arr(j) * qR
			maxabs = max(maxabs, abs(SL(j)), abs(SR(j)))
