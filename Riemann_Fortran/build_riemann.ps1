...
Remove-Item -Path "riemann_solver.exe" -Force -ErrorAction SilentlyContinue

# Write-Host "[2/4] Cleaning previous simulation output files..."
# Remove-Item -Path "riemann_geometry_history_case*.dat" -Force -ErrorAction SilentlyContinue
# Remove-Item -Path "riemann_state_history_case*.dat" -Force -ErrorAction SilentlyContinue
# Remove-Item -Path "riemann_profiles_case*.dat" -Force -ErrorAction SilentlyContinue
...
& gfortran -O3 -march=native -ffast-math -fopenmp -Wno-tabs -J build -c Riemann_solver.f90 -o build\Riemann_solver_omp.o
if ($LASTEXITCODE -ne 0) { throw "Build failed while compiling Riemann_solver.f90." }

& gfortran -fopenmp build\input_vegae_omp.o build\riemann_test_cases_omp.o build\Riemann_solver_omp.o -o riemann_solver.exe
if ($LASTEXITCODE -ne 0) { throw "Build failed while linking riemann_solver.exe." }

Write-Host "[4/4] Build completed: riemann_solver.exe"
