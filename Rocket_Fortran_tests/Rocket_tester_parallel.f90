program rocket_tester
	...
	write(thermo_unit, '(A)') '# t x rho u p T M A P rb G Ts ignited'
	call write_state_snapshot(thermo_unit, 0.0_dp, Ulast, A, P, kglobal, thermal, rb_cycle, G_cycle)
	...
	do while (t < t_end)
		...
		dt = find_dt(Ulast, A, P, dx, cfl, t, kglobal, thermal%ignited, erosive)
		dt = min(dt, t_end - t)
		...
		do while (t >= next_print_time)
			call write_state_snapshot(thermo_unit, t, Ulast, A, P, kglobal, thermal, rb_cycle, G_cycle)
...
contains
	...
	subroutine configure_openmp_runtime()
		...
		subroutine write_state_snapshot(unit_id, t_snap, U_snap, A_snap, P_snap, k_snap, thermal_snap, rb_cycle, G_cycle)
		...
		do j = 1, nx
			...
			if (j<=3 .or. j>=nx-3) then
				...
				write(unit_id, '(13(ES24.14E3,1X))') t_snap, xlist(j), rho_loc(j), &
					u_loc(j), p_loc(j), T_loc(j), mach_loc(j), A_snap(j), P_snap(j), &
	...
	subroutine hllc_flux(U, A_loc, k, fm, fp)
		...
		subroutine erosive_burning(U, A_loc, P_loc, k, erosive_on, rb, G)
			real(dp), intent(in) :: U(nvar, nx), A_loc(nx), P_loc(nx), k(nx)
	...
	subroutine source_term(U, A_loc, P_loc, ignited, dx_local, t_local, k, erosive_on, S, rb, G)
		...
		G = 0.0_dp
		t0 = omp_get_wtime()
		call erosive_burning(U, A_loc, P_loc, k, erosive_on, rb, G)
		...
		real(dp) function find_dt(U, A_loc, P_loc, dx_local, cfl_local, t_local, k, ignited, erosive_on) result(dt_out)
			real(dp), intent(in) :: U(nvar, nx), A_loc(nx), P_loc(nx), dx_local, cfl_local, t_local, k(nx)
			logical, intent(in) :: ignited(nx - 7), erosive_on
			real(dp) :: SL(nx - 5), SR(nx - 5), maxabs
			real(dp) :: Sdt(3, nx - 6), rbdt(nx - 7), Gdt(nx - 7)
			real(dp) :: dt_convective, dt_source, dt_geometry, radius_value
			...
			call source_term(U, A_loc, P_loc, ignited, dx_local, t_local, k, erosive_on, Sdt, rbdt, Gdt)
			...
			do j = 1, nx - 7
				if (rbdt(j) > 0.0_dp) then
					radius_value = sqrt(max(A_loc(j + 3), area_floor) / acos(-1.0_dp))
					dt_geometry = min(dt_geometry, geometry_cfl * radius_value / rbdt(j))
				end if
			end do
	...
	subroutine geometry_from_radius(radius_inner, A_template, P_template, A_out, P_out)
		real(dp), intent(in) :: radius_inner(nx - 7), A_template(nx), P_template(nx)
		real(dp), intent(out) :: A_out(nx), P_out(nx)

		if (any(.not. ieee_is_finite(radius_inner)) .or. any(radius_inner <= 0.0_dp)) then
			error stop 'Invalid port radius in SSPRK geometry stage.'
		end if
		A_out = A_template
		P_out = P_template
		A_out(4:nx - 4) = acos(-1.0_dp) * radius_inner * radius_inner
		P_out(4:nx - 4) = 2.0_dp * acos(-1.0_dp) * radius_inner
		call fill_geometry_ghosts(A_out, P_out)
	end subroutine geometry_from_radius
	...
	subroutine ssprk45(U, A_loc, P_loc, state, dt_local, dx_local, t_local, k, erosive_on, Unp1, Anew, Pnew, rb_cycle, G_cycle)
		...
		real(dp) :: P0(nx), P1(nx), P2(nx), P3(nx), P4(nx)
		...
		real(dp) :: r0(nx - 7), r1(nx - 7), r2(nx - 7), r3(nx - 7), r4(nx - 7), rnew(nx - 7)
		...
		A0 = A_loc
		P0 = P_loc
		r0 = sqrt(max(A0(4:nx - 4), area_floor) / acos(-1.0_dp))

		if (t_local < 0.35_dp) then
			call update_ignition_thermal_state(U0, A0, P0, state, dt_local, kwork)
		else
		...
		t0 = omp_get_wtime()
		call source_term(U0, A0, P0, state%ignited, dx_local, t_local, kwork, erosive_on, S, rb1, G1)
		...
		r1 = r0 + 0.391752226571890_dp * dt_local * rb1
		call geometry_from_radius(r1, A0, P0, A1, P1)
		...
		prof_call_hllc = prof_call_hllc + 1
		t0 = omp_get_wtime()
		call source_term(U1, A1, P1, state%ignited, dx_local, t_local + c2 * dt_local, kwork, erosive_on, S, rb2, G2)
		prof_source = prof_source + (omp_get_wtime() - t0)
		prof_call_source = prof_call_source + 1
		k2 = -(fp - fm) / dx_local + S
		do j = 4, nx - 3
		...
		r2 = 0.444370493651235_dp * r0 + 0.555629506348765_dp * r1 + &
			0.368410593050371_dp * dt_local * rb2
		call geometry_from_radius(r2, A0, P0, A2, P2)
		...
		prof_call_hllc = prof_call_hllc + 1
		t0 = omp_get_wtime()
		call source_term(U2, A2, P2, state%ignited, dx_local, t_local + c3 * dt_local, kwork, erosive_on, S, rb3, G3)
		prof_source = prof_source + (omp_get_wtime() - t0)
		prof_call_source = prof_call_source + 1
		k3 = -(fp - fm) / dx_local + S
		do j = 4, nx - 3
		...
		r3 = 0.620101851488403_dp * r0 + 0.379898148511597_dp * r2 + &
			0.251891774271694_dp * dt_local * rb3
		call geometry_from_radius(r3, A0, P0, A3, P3)
		...
		prof_call_hllc = prof_call_hllc + 1
		t0 = omp_get_wtime()
		call source_term(U3, A3, P3, state%ignited, dx_local, t_local + c4 * dt_local, kwork, erosive_on, S, rb4, G4)
		prof_source = prof_source + (omp_get_wtime() - t0)
		prof_call_source = prof_call_source + 1
		k4 = -(fp - fm) / dx_local + S
		do j = 4, nx - 3
		...
		r4 = 0.178079954393132_dp * r0 + 0.821920045606868_dp * r3 + &
			0.544974750228521_dp * dt_local * rb4
		call geometry_from_radius(r4, A0, P0, A4, P4)
		...
		prof_call_hllc = prof_call_hllc + 1
		t0 = omp_get_wtime()
		call source_term(U4, A4, P4, state%ignited, dx_local, t_local + c5 * dt_local, kwork, erosive_on, S, rb5, G5)
		prof_source = prof_source + (omp_get_wtime() - t0)
		prof_call_source = prof_call_source + 1
		k5 = -(fp - fm) / dx_local + S
		...
		rnew = r0 + dt_local * rb_cycle
		call geometry_from_radius(rnew, A0, P0, Anew, Pnew)
