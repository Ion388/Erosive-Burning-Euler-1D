...
def load_state_history(file_path: Path):
    ...
    if data.shape[1] != 13:
        raise ValueError(
            f"Expected 13 columns (t x rho u p T M A P rb G, Ts, ignited), got {data.shape[1]} columns."
    ...
    reshaped = data.reshape(nt, nx, 13)
    ...
    fields = {
        ...
        "Ts": reshaped[:, :, 11],  # Extract Ts from inner spatial points only. 
        "ignited": reshaped[:, :, 12],  # Extract ignited from inner spatial points only. 
    }
...
def main():
    state_files = sorted(Path(".").glob("rocket_state_history_case*.dat"))
    if not state_files:
        raise FileNotFoundError(
            "No files matching 'rocket_state_history_case*.dat' were found in the current directory."
        )

    state_file = state_files[0]
    print(f"Using state file: {state_file}")

    exit_files = sorted(Path(".").glob("rocket_exit_history_case*.dat"))
    if not exit_files:
        raise FileNotFoundError(
            "No files matching 'rocket_exit_history_case*.dat' were found in the current directory."
        )
