program rocket_tester
	...
	if (A(nx-4) < A2nozzle/epsnozzle) then
		error stop 'Port area is smaller than nominal nozzle throat area.'
	end if
...
contains
	...
	subroutine max_wave_speed_toro(U, A_loc, k, SL, SR, maxabs)
		...
		do j = 1, n_ifaces
			...
			if (max(abs(SL(j)), abs(SR(j))) > 1.0e7_dp) then
				write(*,'(A,I0,A,ES14.6)') &
					'Extreme interface: j=', j, &
					' xface=', 0.5_dp*(xlist(j+2) + xlist(j+3))

				write(*,'(A,6(ES14.6,1X))') &
					'L rho,u,p,a,q,pstar=', &
					rhoL_arr(j), velL_arr(j), pL_arr(j), &
					aL_arr(j), qL, pstarr

				write(*,'(A,6(ES14.6,1X))') &
					'R rho,u,p,a,q,pstar=', &
					rhoR_arr(j), velR_arr(j), pR_arr(j), &
					aR_arr(j), qR, pstarr
			end if
	...
	subroutine source_term(U, A_loc, Pburn_loc, Pwet_loc, ignited, dx_local, t_local, &
		...
		real(dp) function find_dt(U, A_loc, Pburn_loc, Pwet_loc, web_loc, table, &
			...
			integer :: j
			real(dp) :: maxabs_recon, maxabs_cell
			real(dp) :: lambda_cell(nx)
			integer :: imax_cell

			call max_wave_speed_toro(U, A_loc, k, SL, SR, maxabs_recon)
			...
			do j = 1, nx
				...
				lambda_cell(j) = abs(velocity_value) + sound_speed_value
			end do

			maxabs_cell = maxval(lambda_cell)
			imax_cell = maxloc(lambda_cell, dim=1)
			maxabs = max(maxabs_recon, maxabs_cell)

			if (maxabs > 1.0e7_dp) then
				write(*,'(A,ES14.6,A,ES14.6,A,ES14.6,A,I0,A,ES14.6)') &
					't=', t_local, &
					' recon=', maxabs_recon, &
					' cell=', maxabs_cell, &
					' cell index=', imax_cell, &
					' x=', xlist(imax_cell)
			end if
			...
			! if (min(dt_convective, dt_source, dt_geometry) < 1.0e-8_dp) then
			! 	write(*,'(A,ES14.6,4(A,ES14.6),A,I0)') &
			! 		't=', t_local, &
			! 		' dt_conv=', dt_convective, &
			! 		' dt_source=', dt_source, &
			! 		' dt_geom=', dt_geometry, &
			! 		' maxwave=', maxabs, &
