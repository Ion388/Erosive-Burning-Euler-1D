...
contains
	...
	subroutine enforce_positive_state(U, A_loc, k)
		...
		!$omp   shared(U, A_loc, k, rho_floor, p_floor) &
		!$omp   private(j, Ause, rho_val, rhoA, vel, Eint, pval, kminus1) &
		!$omp   schedule(static)
		do j = 1, nx
	...
	subroutine primitives(Uin, Ain, kin, rho_out, vel, pval, Eout, a)
		...
		!$omp parallel do default(shared) &
		!$omp   shared(Uin, Ain, kin, rho_out, vel, pval, Eout, a, nloc, same_size, rho_floor, p_floor) &
		!$omp   private(j, Ause_j, kminus1, u2, a2) &
		!$omp   schedule(static)
		do j = 1, nloc
			if (same_size) then
				Ause_j = Ain(j)
	...
	subroutine max_wave_speed_toro(U, A_loc, k, SL, SR, maxabs)
		...
		!$omp parallel do default(shared) &
		!$omp   shared(SL, SR, rhoL_arr, velL_arr, pL_arr, aL_arr, &
		!$omp          rhoR_arr, velR_arr, pR_arr, aR_arr, khat, n_ifaces, p_floor) &
		!$omp   private(j, khat_safe, kh_m1, kh_p1, inv2k, gexp, pexp, &
		!$omp           pL_gamma, pR_gamma, base_num, base_den, base, pstarr, coeff, qL, qR) &
		!$omp   reduction(max:maxabs) &
		!$omp   schedule(static)
		do j = 1, n_ifaces
	...
	subroutine euler_flux(Uin, A_loc, k, flux)
		...
		!$omp parallel do default(shared) &
		!$omp   shared(Uin, A_loc, rho_arr, vel_arr, pval_arr, Eout_arr, flux, nloc, same_size) &
		!$omp   private(j, Ause_j) &
		!$omp   schedule(static)
		do j = 1, nloc
			if (same_size) then
				Ause_j = A_loc(j)
	...
	subroutine detect_troubled_interfaces(U, A_loc, k, troubled)
		...
		!$omp parallel do default(shared) &
		!$omp   shared(UL, UR, A_loc, k, troubled, n_ifaces, rho_floor, p_floor) &
		!$omp   private(j, Ause_l, Ause_r, rhoL_u, rhoR_u, rhoL_val, rhoR_val, &
		!$omp           velL2, velR2, EL, ER, kLm1, kRm1, pL_u, pR_u, psum, jump) &
		!$omp   schedule(static)
		do j = 1, n_ifaces
	...
	subroutine hlle_flux(U, A_loc, k, fm, fp)
		...
		!$omp parallel do default(shared) &
		!$omp   shared(fhlle, fL, fR, UL, UR, SL, SR, n_ifaces) &
		!$omp   private(j, denom_val) &
		!$omp   schedule(static)
		do j = 1, n_ifaces
	...
	subroutine hllc_flux(U, A_loc, k, fm, fp)
		...
		!$omp parallel do default(shared) &
		!$omp   shared(f, fL, fR, UL, UR, UstarL, UstarR, fstarL, fstarR, &
		!$omp          SL, SR, Sstar, A_loc, rhoL_arr, velL_arr, pL_arr, eL_arr, &
		!$omp          rhoR_arr, velR_arr, pR_arr, eR_arr, n_ifaces) &
		!$omp   private(j, Ause, denom_sstar, denom_l, denom_r, denom_e_l, denom_e_r) &
		!$omp   schedule(static)
		do j = 1, n_ifaces
		...
		!$omp parallel do default(shared) &
		!$omp   shared(fblend, f, fhlle, fL, fR, UL, UR, SL, SR, theta, n_ifaces) &
		!$omp   private(j, denom_val) &
		!$omp   schedule(static)
		do j = 1, n_ifaces
	...
	subroutine erosive_burning(U, A_loc, P_loc, dt_local, k, rb)
		...
		rb = 0.0_dp
		!$omp parallel do default(shared) &
		!$omp   private(j, G, Dh, rb0, r_iter, expo, re_scale, r_new, it) &
		!$omp   schedule(static)
		do j = 1, nx - 7
			G = rho_arr(j + 3) * vel_arr(j + 3)
			Dh = 4.0_dp * A_loc(j + 3) / max(P_loc(j + 3), 1.0e-20_dp)
	...
	subroutine update_ignition_thermal_state(U, A_loc, P_loc, state, dt_local, k)
		...
		if (state%n_hist > 0) then
			do h = 1, state%n_hist
				...
				!$omp parallel do default(shared) &
				!$omp   shared(history_term, state, kernel, h, n_inner) &
		...
		!$omp parallel do default(shared) &
		!$omp   shared(state, rho_arr, vel_arr, pval_arr, A_loc, P_loc, &
		!$omp          history_term, Ts_new, q_new, coeff, sqrt_dt, t_new, n_inner) &
		!$omp   private(j, rho_i, p_i, Rloc, muloc, Kloc, cploc, &
		!$omp           Tg, Dh, Re, Pr, Nu, htc, acoef) &
		!$omp   schedule(static)
		do j = 1, n_inner
			rho_i = rho_arr(j + 3)
			p_i = pval_arr(j + 3)
	...
	subroutine source_term(U, A_loc, P_loc, ignited, dx_local, t_local, k, erosive_on, S, rb)
		...
		!$omp parallel do default(shared) shared(Ainterface, A_loc) private(j) schedule(static)
		do j = 1, nx - 5
			Ainterface(j) = 0.5_dp * (A_loc(j + 3) + A_loc(j + 2))
		end do
		!$omp end parallel do

		!$omp parallel do default(shared) shared(dAdx, Ainterface, dx_local) private(j) schedule(static)
		do j = 1, nx - 6
		...
		S = 0.0_dp
		!$omp parallel do default(shared) shared(S, pval_arr, dAdx) private(j) schedule(static)
		do j = 1, nx - 6
		...
		if (erosive_on) then
			...
			!$omp parallel do default(shared) shared(S, rb, P_loc, ignited) private(j) schedule(static)
			do j = 1, nx - 7
	...
	subroutine ap_map_vec(A0, dtrb, A1, P1)
		...
		!$omp parallel do default(shared) shared(A0, dtrb, A1, P1) private(j, r0, r1) schedule(static)
		do j = 1, size(A0)
	...
	subroutine ssprk45(U, A_loc, P_loc, state, dt_local, dx_local, t_local, k, erosive_on, Unp1, Anew, Pnew)
		...
		k1 = -(fp - fm) / dx_local + S
		!$omp parallel do default(shared) shared(U1, U0, k1, dt_local) private(j) schedule(static)
		do j = 4, nx - 3
		...
		k2 = -(fp - fm) / dx_local + S
		!$omp parallel do default(shared) shared(U2, U0, U1, k2, dt_local) private(j) schedule(static)
		do j = 4, nx - 3
		...
		k3 = -(fp - fm) / dx_local + S
		!$omp parallel do default(shared) shared(U3, U0, U2, k3, dt_local) private(j) schedule(static)
		do j = 4, nx - 3
		...
		k4 = -(fp - fm) / dx_local + S
		!$omp parallel do default(shared) shared(U4, U0, U3, k4, dt_local) private(j) schedule(static)
		do j = 4, nx - 3
		...
		!$omp parallel do default(shared) shared(Unp1, U2, U3, U4, k4, k5, dt_local) private(j) schedule(static)
		do j = 4, nx - 3
