program rocket_tester
	...
	real(dp) :: rho(nx), u(nx), p_prof(nx), E(nx), asnd(nx), rb_cycle(nx - 7)
	real(dp) :: mach(nx)
	...
	write(thermo_unit, '(A)') '# t x rho u p T M A P rb_cycle'
	call write_state_snapshot(thermo_unit, 0.0_dp, Ulast, A, P, kglobal, rb_cycle)
	...
	do while (t < t_end)
		...
		call ssprk45(Ulast, A, P, thermal, dt, dx, t, kglobal, erosive, Unext, Anew, Pnew, rb_cycle)
		A = Anew
		P = Pnew
		do i = 4, nx - 4
		...
		do while (t >= next_print_time)
			call write_state_snapshot(thermo_unit, t, Ulast, A, P, kglobal, rb_cycle)
...
contains
	...
	subroutine write_state_snapshot(unit_id, t_snap, U_snap, A_snap, P_snap, k_snap, rb_cycle)
		integer, intent(in) :: unit_id
		real(dp), intent(in) :: t_snap
		real(dp), intent(in) :: U_snap(nvar, nx), A_snap(nx), P_snap(nx), k_snap(nx), rb_cycle(nx - 7)
		...
		do j = 1, nx
			...
			if (j<=3 .or. j>=nx-3) then
				write(unit_id, '(10(ES24.14E3,1X))') t_snap, xlist(j), rho_loc(j), u_loc(j), p_loc(j), T_loc(j), mach_loc(j), A_snap(j), P_snap(j), 0.0_dp
			else
				! write (*, '(A, I0, A, ES24.14E3)') 'j = ', j, '  rb_cycle = ', rb_cycle(j-3)
				write(unit_id, '(10(ES24.14E3,1X))') t_snap, xlist(j), rho_loc(j), u_loc(j), p_loc(j), T_loc(j), mach_loc(j), A_snap(j), P_snap(j), rb_cycle(j-3)
			end if
		end do
		write(unit_id, '(A)') ''
	...
	subroutine apply_atmospheric_outlet(U, A_loc, k)
		...
		if (u_in >= 0.0_dp) then
			rho_g = rho_in * (p0/p_in) ** (1.0_dp/k_in)
			u_g = u_in
			p_g = p0
		else
			rho_g = rho0
	...
	subroutine apply_nozzle_outlet(U, A_loc, k)
		...
		! real(dp) :: rho_s, u_s, E_s, k_s, p_s, R_s, T_s, a_s, M_s
		real(dp) :: T0_f, p0_f, Athroat, area_ratio, critical_ratio, M_compare
		real(dp) :: M_s(3), T_s(3), p_s(3), rho_s(3), u_s(3), E_s(3)
		...
		if (p0 / p0_f >= critical_ratio) then
			call apply_atmospheric_outlet(U, A_loc, k)
			return
		end if

	
		area_ratio = A_loc(nozzle_state_idx) / Athroat
		M_s(1) = solve_area_mach(area_ratio, k_f, .false.) + 0.1_dp
		! write(*, '(A, ES14.6)') 'M_s = ', M_s
		T_s(1) = T0_f / (1.0_dp + 0.5_dp * (k_f - 1.0_dp) * M_s(1) * M_s(1))
		p_s(1) = p0_f / (1.0_dp + 0.5_dp * (k_f - 1.0_dp) * M_s(1) * M_s(1)) ** (k_f / (k_f - 1.0_dp))
		rho_s(1) = p_s(1) / max(R_f * T_s(1), 1.0e-20_dp)
		u_s(1) = M_s(1) * sqrt(k_f * R_f * T_s(1))

		area_ratio = A_loc(nozzle_state_idx+1) / Athroat
		M_s(2) = solve_area_mach(area_ratio, k_f, .false.) + 0.1_dp
		T_s(2) = T0_f / (1.0_dp + 0.5_dp * (k_f - 1.0_dp) * M_s(2) * M_s(2))
		p_s(2) = p0_f / (1.0_dp + 0.5_dp * (k_f - 1.0_dp) * M_s(2) * M_s(2)) ** (k_f / (k_f - 1.0_dp))
		rho_s(2) = p_s(2) / max(R_f * T_s(2), 1.0e-20_dp)
		u_s(2) = M_s(2) * sqrt(k_f * R_f * T_s(2))

		area_ratio = A_loc(nozzle_state_idx+2) / Athroat
		M_s(3) = 1
		T_s(3) = T0_f / (1.0_dp + 0.5_dp * (k_f - 1.0_dp) * M_s(3) * M_s(3))
		p_s(3) = p0_f / (1.0_dp + 0.5_dp * (k_f - 1.0_dp) * M_s(3) * M_s(3)) ** (k_f / (k_f - 1.0_dp))
		rho_s(3) = p_s(3) / max(R_f * T_s(3), 1.0e-20_dp)
		u_s(3) = M_s(3) * sqrt(k_f * R_f * T_s(3))
	...
	subroutine supersonic_properties(U, A_loc, k, M_exit, T_exit, p_exit, rho_exit, u_exit, E_exit, thrust)
		...
		integer, parameter :: nozzle_state_idx = nx - 2
		real(dp) :: rho_s, u_s, E_s, k_s, p_s, R_s, T_s, a_s, M_s
		real(dp) :: T0_s, p0_s, Athroat, area_ratio
		real(dp) :: M_exit, T_exit, p_exit, rho_exit, u_exit, E_exit, thrust
		real(dp) :: M_compare

		rho_s = max(U(1, nozzle_state_idx) / max(A_loc(nozzle_state_idx), area_floor), rho_floor)
		u_s = U(2, nozzle_state_idx) / max(rho_s * A_loc(nozzle_state_idx), 1.0e-20_dp)
		E_s = U(3, nozzle_state_idx) / max(rho_s * A_loc(nozzle_state_idx), 1.0e-20_dp)
		k_s = k(nozzle_state_idx)
		p_s = rho_s * max(k_s - 1.0_dp, 1.0e-8_dp) * (E_s - 0.5_dp * u_s * u_s)
		R_s = gas_constant_from_k(k_s)
		T_s = p_s / max(rho_s * R_s, 1.0e-20_dp)
		a_s = sqrt(max(k_s * p_s / rho_s, a2_floor))
		M_s = max(u_s / a_s, 0.0_dp)

		T0_s = T_s * (1.0_dp + 0.5_dp * (k_s - 1.0_dp) * M_s * M_s)
		p0_s = p_s * (1.0_dp + 0.5_dp * (k_s - 1.0_dp) * M_s * M_s) ** (k_s / (k_s - 1.0_dp))

		Athroat = A2nozzle / epsnozzle
		area_ratio = A_loc(fictitious_cell_idx) / Athroat
		M_compare = solve_area_mach(area_ratio, k_s, .false.)

		if (M_s < M_compare) then
			M_exit = 0
			T_exit = 0
			p_exit = 0
			rho_exit = 0
		...
		T_exit = T0_s / (1.0_dp + 0.5_dp * (k_s - 1.0_dp) * M_exit * M_exit)
		p_exit = p0_s / (1.0_dp + 0.5_dp * (k_s - 1.0_dp) * M_exit * M_exit) ** (k_s / (k_s - 1.0_dp))
		rho_exit = p_exit / max(R_s * T_exit, 1.0e-20_dp)
		u_exit = M_exit * sqrt(k_s * R_s * T_exit)
		E_exit = p_exit / (k_s - 1.0_dp) + 0.5_dp * rho_exit * u_exit * u_exit
		thrust = rho_exit * u_exit * u_exit * A2nozzle + (p_exit - p0) * A2nozzle
		! write(*, '(A, ES14.6)') 'M_exit = ', M_exit
	...
	subroutine ssprk45(U, A_loc, P_loc, state, dt_local, dx_local, t_local, k, erosive_on, Unp1, Anew, Pnew, rb_cycle)
		real(dp), intent(in) :: U(nvar, nx), A_loc(nx), P_loc(nx), dt_local, dx_local, t_local, k(nx)
		type(thermal_state_t), intent(inout) :: state
		logical, intent(in) :: erosive_on
		real(dp), intent(out) :: Unp1(nvar, nx), Anew(nx), Pnew(nx), rb_cycle(nx - 7)
